...
roleService.saveRoleInfo = async (user, roleId, menuIds) => {
    ...
    for (let i = 0; i < userRoles.length; i++) {
        userMenuAll = userMenuAll.concat(await userRoles[i].getSys_permissions())
    }
...
roleService.getRoleInfo = async (user, roleId) => {
    ...
    let userMenuAll = []
    for (let i = 0; i < userRoles.length; i++) {
        userMenuAll = userMenuAll.concat(await userRoles[i].getSys_permissions({order: ['orderNum']}))
    }
    userMenuAll = objArrayDoWeight(userMenuAll)
    const role = await SysRole.findByPk(roleId)
    let roleMenu = []
    let temp = await role.getSys_permissions({where: {type: 1}})
    temp.forEach(item => {
